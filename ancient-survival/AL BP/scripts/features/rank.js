...
export function setRank(player) {
  ...
  let listRank = [...rankList]
  let customRank = null
  if (player.hasTag('customrank')) {
    customRank = new PlayerDatabase(player, 'CustomRank').get()
    
    if (customRank && !listRank.includes(customRank)) {
      listRank.push(customRank)
    }
  }
    if (!listRank.length) {
    ...
    const form = new ModalFormData()
      .title('Set Your Rank')
      .dropdown('Pilih rank yang kamu miliki:', listRank)
    
    OpenUI.force(player, form).then(r => {
      if (r.canceled) return
      
      const selectedRank = listRank[r.formValues[0]]
      let config
      if (selectedRank === d) {
        config = CUSTOM_CONFIG
      } else {
        config = RANK_CONFIG[selectedRank]
      }
      if (!config) return
